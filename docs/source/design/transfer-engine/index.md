...
- `MC_RDMA_DATA_DIRECT` Set to `1` to pair PCIe BAR1 DMA-BUF export with NVIDIA Data Direct registration for CUDA device memory in the classic RDMA transport. Default: disabled; overrides `WITH_NVIDIA_PEERMEM` when enabled. Included in CUDA builds even with older RDMA headers; requires compatible hardware, drivers, firmware, and runtime `libmlx5` ([platform requirements](https://docs.nvidia.com/multi-node-nvlink-systems/grace-blackwell-cx8-gpudirect-rdma-guide/platform_software_and_configuration.html)). Unsupported configurations fail without fallback. Restrict the engine to compatible NICs using the existing device filter or custom topology: every selected NIC, including fallback NICs, registers every buffer, and affinity must use the Data Direct PCIe function. NIC selection is not automatic. Host memory registration is unchanged. Does not configure TENT or the IBGDA Device API.
